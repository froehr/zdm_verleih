...
if (mysql_num_rows($ueberzogen) != 0) {
		while ($row = mysql_fetch_object($ueberzogen)) {
			...
			$tpl = tpl_replace_once("emailadresse1", '<a href="mailto:'.$row->email.'?cc=zdm@uni-muenster.de&bcc=&subject=Überzogene Ausleihe&body=Hallo '.$row->vorname.',%0D%0D
			...
			$tpl = tpl_replace_once("anzahlverlängerung1", $bis);
			$tpl = tpl_replace_once("bisherigeverspätungen1", $row->bis);
...
if (mysql_num_rows($faellig) != 0) {
		while ($row = mysql_fetch_object($faellig)) {
			...
			$tpl = tpl_replace_once("anzahlverlängerung2", $row->bis);
			$tpl = tpl_replace_once("bisherigeverspätungen2", $row->bis);
...
if (mysql_num_rows($nochzeit) != 0) {
		while ($row = mysql_fetch_object($faellig)) {
			...
			$tpl = tpl_replace_once("anzahlverlängerung3", $row->bis);
			$tpl = tpl_replace_once("bisherigeverspätungen3", $row->bis);
